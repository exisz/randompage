...
- Bookmarks saved-passage cards also expose a single-passage Markdown export for Obsidian/Notion-style vaults. It copies Markdown to clipboard first and falls back to a `.md` download; the generated file preserves title/author/chapter, canonical RandomPage URL, tags, collection names, the saved excerpt, the bookmark private note, and line-level private thoughts/annotations. This is client-local over existing user-owned RandomPage saved passages only: no Notion/Obsidian API integration, summaries, sync, social highlights, or new content source.
...
| `check-markdown-export-policy.mjs` | PLANET-3329 | 静态回归检查 Bookmarks 单条 saved passage Markdown export、clipboard-first + `.md` fallback、excerpt/title/author/canonical URL/tags/collections/private note/line-level thoughts、no-summary/no-integration boundary。 |
