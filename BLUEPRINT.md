...
│  │    /api/reading/insights-wrapup → private 7/30-day activity wrap-up over browsing_events, push_history, bookmarks, passage_reviews, reading_paths, user_preferences, and passage metadata/tags │ │
...
│          user_preferences, saved_books, reading_paths,              │
...
| private insights wrap-up | `/api/reading/insights-wrapup` derives signed-in 7/30-day metrics from existing user-owned RandomPage records only: opened pages, pushed pages, bookmarks, reviews, reading paths, preferences, and passage metadata/tags. `/insights` renders top books/authors/tags, discovered sources, and deterministic revisit suggestions without external LLMs, summaries, social comparison, or new content sources. |
...
- Web App Manifest files (`manifest.json` and `manifest.webmanifest`) keep the default installed-app launch at `/discover` and expose home-screen shortcuts to existing RandomPage surfaces only: `/today`, `/bookmarks#daily-review-overview`, `/bookmarks`, and `/history?tab=push#push-inbox`. These shortcuts lower friction to daily passages, saved review, saved library, and push inbox without adding summaries, external content, or a new recommendation surface.
...
| `check-preference-calibration-policy.mjs` | PLANET-3555 | 静态回归检查 Settings free-text preference note、`POST /api/preferences/calibration`、deterministic tag matching、private control rows、derived positive/avoid preference rows 与 no external LLM/embedding boundary |
...
| `loc-digitized-books-eval.mjs` | PLANET-3874 | Local Library of Congress Selected Digitized Books evaluation (`pnpm --filter @randompage/app eval:loc-digitized-books`); reads LOC Labs bulk manifest, samples direct `.txt` OCR files, fetches item metadata, emits JSON/Markdown passage-yield artifacts, no production writes |
| `loc-reviewed-queue.mjs` / `loc-reviewed-ingest.mjs` | PLANET-3882 | Review-first LOC Selected Digitized Books ingest path: `queue:loc-reviewed` emits metadata-only candidate/review artifacts with rows defaulting `reviewed:false`; `ingest:loc-reviewed` fetches only `reviewed:true` `.txt` files serially, dry-runs by default, and requires `--apply --ack-reviewed` plus Turso credentials for tiny production inserts (`tags=[]`) after RandomPage length/content filters |
| `harvard-public-domain-corpus-eval.mjs` | PLANET-3911 | Local Harvard Library Public Domain Corpus / Institutional Books 1.0 evaluation (`pnpm --filter @randompage/app eval:harvard-public-domain-corpus`); samples public Hugging Face metadata, attempts tiny gated OCR row fetch only when `HF_TOKEN` is present, emits report/sample artifacts, and never writes Turso or exposes unreviewed text |
| `check-loc-reviewed-ingest-policy.mjs` | PLANET-3882 | Static guard requiring metadata-only queue posture, reviewed allowlist gating, dry-run default, explicit apply acknowledgement, RandomPage passage filters, and package script registration |
...
| `ia-ocr-passage-yield-eval.mjs` | PLANET-3696 | Reviewed IA/OCR passage-yield evaluation (`pnpm --filter @randompage/app eval:ia-ocr-passage-yield`) over rows explicitly marked `reviewed:true` + `allowOcrFetch:true`; fetches IA metadata/OCR plaintext serially, classifies metadata-only/snippet-only/usable outcomes, emits JSON/MD local artifacts, and never writes production DB |
| `check-ia-ocr-passage-yield-policy.mjs` | PLANET-3696 | Static guard for the reviewed passage-yield eval: requires allowlist gating, package scripts, outcome classification, local artifacts, and no Turso/mutation SQL paths |
...
| 2026-07-20 | PLANET-3874: Added local Library of Congress Selected Digitized Books passage-source evaluation (`pnpm --filter @randompage/app eval:loc-digitized-books`). It reads LOC Labs `manifest.txt`, samples direct `.txt` OCR files, fetches LOC item JSON metadata, slices RandomPage-sized candidate snippets, and emits JSON/Markdown yield artifacts without production DB writes, summaries, full-reader UI, or LLM tagging. | Engineer Pod |
